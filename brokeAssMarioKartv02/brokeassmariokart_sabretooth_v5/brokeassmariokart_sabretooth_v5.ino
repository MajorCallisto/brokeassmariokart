...
/* Disable Light*/
Adafruit_NeoPixel strip = Adafruit_NeoPixel(52, NEOPIN, NEO_GRB + NEO_KHZ800);
  /**/
...
void loop()
{
  ...
  if (millis() - lastMessage > messageTimeout){
    ...
    motor_speed_m1 = motor_speed_m2 = 0;
    lastMessage = millis();
  }
...
void updateNeoColors(){
  for (int i = 0; i< strip.numPixels()/2; i++){
    if (last_speed_m1<0){
      strip.setPixelColor(i, strip.Color(0, abs(last_speed_m1)*2, 0));
    }else{
      strip.setPixelColor(i, strip.Color(abs(last_speed_m1)*2, 0, 0));
    }
  }
  for (int i = strip.numPixels()/2; i< strip.numPixels(); i++){
        if (last_speed_m2<0){
      strip.setPixelColor(i, strip.Color(0, abs(last_speed_m2)*2, 0));
    }else{
      strip.setPixelColor(i, strip.Color(abs(last_speed_m2)*2, 0, 0));
    }
  }
  strip.show();
}
